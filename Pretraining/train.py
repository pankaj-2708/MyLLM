...
if not (
    os.path.isfile("/kaggle/working/my_tokenizer.json")
    and os.path.isfile("/kaggle/working/pretraining_dataset.bin")
):
...
class Trainer:
    def __init__(
        ...
        self.lr_scheduler = get_cosine_schedule_with_warmup(
            self.optimiser, warmup_steps, total_tokens
        )
    ...
    def resume_training(self):
        ...
        checkpoint = torch.load(
            "/kaggle/input/datasets/pankajmaulekhi/checkpointer/checkpoint.pth",
            map_location="cpu",
            weights_only=False,
        )
    ...
    def create_optimiser(self):
        decay_params = [
            p for n, p in self.model.named_parameters() if p.dim() >= 2
        ]  # weight matrices only
        no_decay_params = [
            p for n, p in self.model.named_parameters() if p.dim() < 2
        ]  # biases, layernorm, etc.
...
def main(rank, world_size):
    ...
    model = LLM(
        vocab_size=vocab_size, embed_dim=embed_dim, n_layer=n_layer, n_head=n_head
    ).to(rank)
