...
[![Model on HuggingFace](https://img.shields.io/badge/🤗%20HuggingFace-Pankaj121212%2FPM__MiniFinLLM-blue)](https://huggingface.co/Pankaj121212/PM_MiniFinLLM)
...
- **Pre-Layer Normalization** (norm before attention/FFN rather than after)
- **Weight tying** between the token embedding and the output projection layer, reducing parameter count
- **GELU activations** in the FFN with 4× hidden dimension expansion
- **Causal attention** via `torch.nn.functional.scaled_dot_product_attention` with `is_causal=True`

```
vocab_size  = 32,000    embed_dim = 768
...
- ~1.99B words, ~2.36B tokens after BPE tokenization
...
- Checkpointing: saves model, optimizer, scaler, scheduler, and RNG state every 100 steps
...
PM-MiniFinLLM/
├── pretraining/
│   ├── arch.py                  # Model architecture (RoPE, attention, transformer layers)
│   ├── train.py                 # DDP pre-training loop with checkpointing
│   ├── dataset.py               # Binary dataset loader (np.memmap)
│   └── tokenizer_training.py    # BPE tokenizer training on EDGAR corpus
│
├── sft/
│   ├── prepare_dataset.py       # Dataset merging, formatting, train/val/test split
│   └── sft_train.py             # SFTTrainer setup and training
│
├── dpo/
│   └── dpo_train.py             # Custom DPO training loop
│
├── hf_upload/
│   ├── PM_MiniFinLLM/
│   │   ├── arch.py
│   │   ├── configuration_PM_MiniFinLLM.py   # PretrainedConfig subclass
│   │   └── modeling_PM_MiniFinLLM.py        # PreTrainedModel + GenerationMixin
│   └── upload.py                # Hub push script with AutoClass registration
│
└── data/
    ├── download_edgar.py        # EDGAR corpus downloader (2008–2020)
    └── preprocess.py            # Section extraction and binary tokenization
```
...
- 110M parameters is too small for competitive performance on financial NLP benchmarks. This is expected and by design.
- The tokenizer was trained without proper special tokens for chat formatting — a workaround using repurposed vocabulary tokens was implemented for SFT/DPO stages.
- SFT diverged due to model capacity constraints, not implementation bugs (verified via debug forward pass inspecting label masks).
- DPO used a general-purpose preference dataset (not finance-specific) due to lack of available domain-specific data.
